...
public class FileSize {
	...
	public String toString() throws RuntimeException {
		try {
			...
			if (this.longSize >= 0 && this.longSize < SIZE_BT) {
				...
				return MathExtend.divide(this.longSize, SIZE_BT, SACLE) + "KB";
			} else if (this.longSize >= SIZE_KB && this.longSize < SIZE_MB) {
				return MathExtend.divide(this.longSize, SIZE_KB, SACLE) + "MB";
	...
	public static String toSize(long t){
		if(t>0){
			if (t >= 0 && t < SIZE_BT) {
				return t + "B";
			} else if (t >= SIZE_BT && t < SIZE_KB) {
				return MathExtend.divide(t, SIZE_BT, SACLE) + "KB";
			} else if (t >= SIZE_KB && t < SIZE_MB) {
				return MathExtend.divide(t, SIZE_KB, SACLE) + "MB";
			} else if (t >= SIZE_MB && t < SIZE_GB) {
				...
				String result = longs.divide(sizeMB, SACLE,
						BigDecimal.ROUND_HALF_UP).toString();
				// double result=t/(double)SIZE_MB;
				return result + "GB";
			} else {
				BigDecimal longs = new BigDecimal(Double.valueOf(
						t + "").toString());
