 ...
 */
Ext.ns("Ext.ui");
Ext.ui.UploadPanel = Ext.extend(Ext.Panel, {
	initComponent : function(cfg) {
		...
		this.addBtn = Ext.id();
		this.uploadBtn = Ext.id();
		this.stopBtn = Ext.id();
		this.deleteBtn = Ext.id();
		this.tbar = [{
					text : '添加文件',
					iconCls : 'add',
					id : this.addBtn
		...
		this.on('render', function() {
					var em = this.getTopToolbar().items.item(0).el.child('em');
					var placeHolderId = Ext.id();
					this.addBtn = Ext.getCmp(this.addBtn);
					this.uploadBtn = Ext.getCmp(this.uploadBtn);
					this.stopBtn = Ext.getCmp(this.stopBtn);
					this.deleteBtn = Ext.getCmp(this.deleteBtn);
					em.setStyle({
								position : 'relative',
								display : 'block'
							});
		...
		Ext.ui.UploadPanel.superclass.initComponent.apply(this, arguments);
	},
	width : 510,
	height : 200,
	layout : 'fit',
	...
	onUploadComplete : function(file) {
		...
		if (this.getStats().files_queued > 0 && this.uploadStopped == false) {
			...
			me.toggleBtn(false);
			me.linkBtnEvent();
			me.fireEvent('alluploadcomplete');
		}
	},
	...
	formatDelBtn : function(v) {
		return "<a href='#' id='" + v
				+ "'  style='color:blue' class='link-btn'>移除</a>";
	},
...
Ext.reg('uploadpanel', Ext.ui.UploadPanel);
